...
import {
  useGetCartByUserId,
  useUpdateCartByUserId,
} from "../../../hooks/apis/carts";
...
import {
  ...
  useFindUserAddressByUserId,
  useUpdateUserAddress,
} from "../../../hooks/apis/userAddress";
import { v4 as uuidv4 } from "uuid";
import { useAddOrder, useDeleteOrder } from "../../../hooks/apis/order";
import Loading from "../../../components/loading/Loading";
import {
  useGetVoucherByUserId,
  useUpdateApplyVoucher,
} from "../../../hooks/apis/voucher";
...
const Checkout = () => {
  ...
  const { data: listAddressByUser } = useFindUserAddressByUserId({
    userId: user?._id,
  });
  ...
  useEffect(() => {
    if (listAddressByUser?.data) {
      setCheckedAddress(
        listAddressByUser?.data?.find((i) => i.default == true) || null
      );
    }
  ...
  const { voucherId, salePrice, code } = useSelector(
    (state: { voucher: any }) => state.voucher
  );
          ...
          try {
            ...
            const infoAddOrder = {
              ...
              address: `Số nhà/Đường: ${
                checkedAddress.street || "Không xác định"
              } - ${checkedAddress.ward.label} - ${
                checkedAddress.district.label
              } - Thành phố ${checkedAddress.city.label}`,
            ...
            const productsInAOnly = data?.data?.carts.filter((itemA) => {
              ...
              return !listProductCartSelected.some(
                (itemB) =>
                  itemB?.product?._id == itemA?.product?._id &&
                  itemB?.selectedSize == itemA?.selectedSize
              );
            });
            // Cập nhật lại số sản phẩm còn lại trong giỏ hàng
            useUpdateCart({
              userId: user._id,
              carts: productsInAOnly?.map((item) => {
                return {
            ...
            await addNotification({
              ...
              content: `Đơn hàng được tạo vào ${moment(date).format(
                "DD/MM/YYYY HH:mm"
              )}`,
            });
            ...
            if (statusCheckedEmailOrder) {
              const newProducts = await Promise.all(
                res.data.newOrder.products.map(async (product) => {
          ...
          try {
            if (!checkedAddress) {
              ...
              localStorage.setItem(
                "idOrder",
                res?.data?.newOrder?.orderCode.slice(1)
              );
              ...
              localStorage.setItem(
                ...
                JSON.stringify({
                  ...infoAddOrder,
                  voucherId,
                  listProductCartSelected,
                  _id: res?.data?.newOrder?._id,
                })
              );
  ...
  useEffect(() => {
    if (voucherId && userVoucher) {
      setSelectedVoucher(
        userVoucher.find((voucher) => voucher?.codeId == voucherId)
      );
    }
            ...
            <div className="-mx-3 md:flex items-start">
                ...
                <div className="w-full mx-auto  font-light mb-6 ">
                  {listProductCartSelected?.map((item, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        className="w-full flex items-center mb-2 border-b pb-2 "
                      >
                ...
                <div className="mb-6 pb-6 border-b border-gray-200 ">
                  <div className="w-full flex mb-3 items-center">
                    <div className="flex-grow">
                      <span className=" font-medium">
                        <i className="fa-solid fa-ticket mt-[1px] text-[#000000b6] mr-1"></i>
                        Phiếu ưu đãi
                      </span>
                    </div>
                    <div className="pl-3">
                      ...
                      <button
                        className="relative mt-2 font-medium text-sm hover:underline text-[#0000ffc7]"
                        onClick={() => {
                          toggleModal(0, true);
                        }}
                      >
                  ...
                  <div className="w-full flex mb-3 items-center">
                    ...
                    <div className="pl-3">
                      <span className="font-semibold">
                        {salePrice
                          ? formatCurrency(lastPriceInit - salePrice)
                          : formatCurrency(0)}{" "}
                ...
                <div className=" pb-6 border-b border-gray-200 md:border-none  text-xl">
                  <div className="w-full flex items-center">
                    ...
                    <div className="pl-3 text-[red] font-bold">
                      {lastPrice != 0 ? (
                        formatCurrency(
                          lastPriceInit - salePrice + (transportFee || 0)
                        )
                      ) : (
                        <Loading />
                      )}
              ...
              <div className="px-3 md:w-5/12 xl:mt-[-40px]">
                  ...
                  <div className="m-2 leading-7">
                    <div className="text-xl font-medium ">
                      <div className="flex gap-2 text-slate-700 items-center">
                        ...
                        <button
                          onClick={() => setModalChangeAddressOpen(true)}
                          className="text-blue-500 mt-1 text-sm "
                        >
                          {checkedAddress ? "Thay đổi" : "Chọn địa chỉ"}
                        </button>
                        ...
                        >
                          <ul className="w-full text-sm font-thin text-gray-900 bg-white rounded-lg">
                            {dataAddress?.map((item, index) => (
                              <li
                                key={index}
                                className="border-b m-2 border-gray-200 border-x-none rounded-t-lg flex items-center"
                              >
                                <div
                                  onClick={() => setCheckedAddress(item)}
                                  className="flex items-center radio-button ps-3 w-full"
                                >
                                  <input
                                    id={item._id}
                                    type="radio"
                                  ...
                                  <label
                                    htmlFor="address"
                                    className="radio-button__label w-full "
                                  >
                                    <span className="radio-button__custom"></span>
                                    <div className="flex justify-between items-center w-full">
                                      <div className="max-w-[70%] ">
                                        <div>
                                          <span className="font-normal pr-2 border-r-2">
                                            {item?.fullName}
                                          </span>
                                          <span className="pl-2">
                                            (+84) {item?.phoneNumber}
                          ...
                          <button
                            ...
                            className=" text-gray-700 rounded-lg relative m-3 w-36 h-10 cursor-pointer flex items-center border border-blue-500  group  overflow-hidden"
                          >
                          ...
                          >
                            <form
                              action="#"
                              className="pt-3 border-t grid  gap-6"
                            >
                              <div>
                        ...
                        <div>
                          <div className="text-base font-medium">
                            <span className="pr-2 border-r-2">
                              {checkedAddress?.fullName}
                            </span>
                            <span className="pl-2">
                              (+84) {checkedAddress?.phoneNumber}
                            </span>
                            <p>Email: {checkedAddress?.email}</p>
                          </div>
                          <div>
                            Số nhà/đường: {checkedAddress?.street},{" "}
                            {checkedAddress?.ward?.label},{" "}
                            {checkedAddress?.district?.label},{" "}
                            {checkedAddress?.city?.label}
                          </div>
                        </div>
                      </div>
                    )}
                ...
                <div className="cursor-pointer select-none w-full mx-auto rounded-lg bg-white shadow-sm border font-light">
                  <p className=" font-medium m-2 pb-2 border-b">
                  ...
                  <ul className="w-full text-sm font-medium text-gray-900 bg-white rounded-lg">
                    ...
                    <li className=" mx-2 rounded-t-lg">
                      <div className="flex items-center cursor-not-allowed ps-3">
                        <input
                          disabled
                          id="vnpay"
                          type="radio"
                          ...
                          className="radio-button__input pointer-events-none"
                        />
                        <label
                          htmlFor="vnpay"
                          className=" py-3 ms-2 radio-button__label pointer-events-none  "
                        >
                          <span className="radio-button__custom"></span>{" "}
                          <img
                            alt=""
                ...
                <div className="flex items-stretch gap-x-[5px] mt-3">
                  <input
                    id="emailOrder"
                    type="checkbox"
                    checked={statusCheckedEmailOrder}
                    onChange={() =>
                      setStatusCheckedEmailOrder(!statusCheckedEmailOrder)
                    }
                  />
                  <label
                    htmlFor="emailOrder"
                    className="text-sm mt-2 select-none cursor-pointer"
                  >
                    Nhận email xác nhận
                  </label>
                </div>
            ...
            <div className="h-[70vh] overflow-scroll">
              {userVoucher
                ?.sort((a, b) => {
                  ...
                  if (
                    a.apply ||
                    moment().isBefore(moment(a?.expirationDate[0]))
                  ) {
                    if (
                      !b.apply &&
                      moment().isAfter(moment(b?.expirationDate[1]))
                    ) {
                      ...
                      !b.apply &&
                      !moment().isBefore(moment(b?.expirationDate[0]))
                    ) {
                        ...
                        <div className="flex justify-between py-4 px-3 ">
                          <div className="w-[75%]">
                            ...
                            <p className="xl:text-sm text-xs">
                              Thời gian{" "}
                              {moment(item?.expirationDate[0]).format(
                                "DD/MM/YYYY HH:mm"
                              )}{" "}
                              đến{" "}
                              {moment(item?.expirationDate[1]).format(
                                "DD/MM/YYYY HH:mm"
                              )}
                            </p>
                          ...
                          <button className="px-3 w-[20%] ">
                            <input
                              ...
                              disabled={
                                item?.apply ||
                                moment().isBefore(
                                  moment(item?.expirationDate[0])
                                ) ||
                                moment().isAfter(
                                  moment(item?.expirationDate[1])
                                )
                              }
                              onChange={() => {
