...
import {
  setCartQuantity,
  setRepuchaseListProductChecked,
...
const Cart = () => {
  ...
  const { RepuchaseListProductChecked } = useSelector(
  ...
  const handleDelete = async (index: number, id, size) => {
    ...
    setListProductChecked((pre) =>
      pre.filter(
        (item, idx) => item.product._id !== id || item.selectedSize !== size
      )
    );
  ...
  const onChangeQ = async (value: number, index: number, item) => {
    const updatedListCart = listCart.map((cartItem, idx) =>
      idx == index ? { ...cartItem, selectedQuantity: value } : cartItem
    );
    setListCart(updatedListCart);
    ...
    const max = item.product.sizes.find(
      (size) => size.size === item.selectedSize
    )?.quantity;
    if (value >= max) {
      const updatedListCart = listCart.map((cartItem, idx) =>
        idx == index ? { ...cartItem, selectedQuantity: max } : cartItem
      );
      setListCart(updatedListCart);
  ...
  const handleSelectAll = () => {
    ...
    if (!selectAllChecked) {
      ...
      setListProductChecked(
        listCart.filter(
          (item) =>
            item.product && item.product.sizes.some((s) => s.quantity > 0)
        )
      );
    } else {
            ...
            <div className="">
              <nav className="text-xs bg-neutral-50 py-2 p-1 font-semibold uppercase border">
                <ul className="flex  font-medium text-base text-center">
                  <li className=" w-[10%]  text-sm ">#</li>
                  <li className=" w-[60%] text-left ">Sản Phẩm</li>
                  <li className=" w-[40%] xl:block hidden ">Số Lượng</li>
                  <li className=" w-[40%] xl:block hidden ">Phân Loại</li>
                  <li className=" w-[40%] xl:block hidden ">Tổng Tiền</li>
                  <li className=" xl:w-[20%] w-[30%]"></li>
                </ul>
              </nav>
              <div className="my-3">
                    ...
                    <div
                        ...
                        if (
                          item?.product?.sizes.find(
                            (s) =>
                              ...
                              s.quantity >= item.selectedQuantity
                          ) &&
                          item?.product?.sizes.some((s) => s.quantity > 0) &&
                          item?.product != null
                        ) {
                      ...
                      className={`relative select-none ${
                        item?.product?.sizes.find(
                          ...
                          ? "bg-neutral-50"
                          : "bg-gray-400"
                      } bg-neutral-50 mt-2 cursor-pointer flex  text-center p-1 hover:bg-slate-100`}
                    >
                      <div className="py-4 w-[10%] ">
                        <input
                          ...
                          checked={listProductChecked?.some(
                            (product) =>
                              product?.selectedSize == item?.selectedSize &&
                              item?.product?._id == product?.product?._id
                          )}
                      ...
                      <div className=" xl:w-[60%] w-full flex  xl:text-base text-sm">
                        ...
                        item.product.images.length > 0 ? (
                          <img
                            src={
                              typeof item?.product?.images?.[0] === "string"
                                ...
                                : item?.product?.images?.[0]?.response
                                    ?.urls?.[0] ||
                          ...
                          <img
                            src="https://thcsgiangvo-hn.edu.vn/wp-content/uploads/2023/09/anh-loading-1.jpg"
                            alt="default-img"
                            className="xl:w-[28%] xl:h-[100%] w-20"
                          />
                        )}
                        ...
                        <div className="flex  justify-between  w-full xl:hidden">
                          <div className=" w-[45%]">
                            <div className="text-left ml-3 xl:hidden block">
                              {" "}
                              {item?.product?.name}{" "}
                            </div>
                            <div className="text-left ml-3 xl:hidden block ">
                            ...
                            <div className="text-left ml-3 xl:hidden block  text-[red] font-medium">
                              {" "}
                              {formatCurrency(
                                item?.product?.sizes?.find(
                                  (s) => s.size == item?.selectedSize
                          ...
                          <div className=" w-[40%]">
                            ...
                            <div
                              ...
                              className=" xl:hidden block mt-2"
                            >
                              <svg
                      ...
                      <div className="w-[40%]  py-4 xl:block hidden xl:text-base text-sm">
                        <span onClick={(event) => event.stopPropagation()}>
                          <InputNumber
                            min={1}
                            value={item?.selectedQuantity}
                            onChange={(value) => onChangeQ(value, i, item)}
                            className="xl:w-[80px] w-[50px]"
                          />
                        </span>
                      </div>
                      ...
                      <div
                        onClick={() =>
                          handleDelete(i, item.product._id, item.selectedSize)
                        }
                        className=" w-[20%] py-4 xl:block hidden xl:text-base text-sm"
                      >
                        <svg
                          className="w-6 h-6 mx-auto"
                          fill="none"
                      ...
                      !item?.product?.sizes.some((s) => s.quantity > 0) &&
                      item?.product != null ? (
                        <div className="absolute top-0 right-0">Hết hàng</div>
                      ) : undefined}
              ...
              <div className=" flex justify-between">
                ...
                <div className="xl:w-96 w-full p-2">
                  <div className="flex justify-between xl:text-base text-sm">
                    <h3 className=" mt-[10px]  flex ">
                      <i className="fa-solid fa-ticket mt-[1px] text-[#000000b6]"></i>
                      <span className="ml-1"> Phiếu ưu đãi</span>
                    </h3>
                    <button
              ...
              <div className="flex justify-between p-2">
                <div className="flex">
                  <button
                    ...
                    className=" xl:p-2 p-1 flex xl:w-36 w-16"
                  >
                    <input
                      type="checkbox"
                      className="my-2"
                    ...
                    />
                    <div className="xl:ml-3 ml-1 flex xl:text-base text-sm  mt-1">
                      <p className="xl:text-base text-sm xl:block hidden mr-1">
                        Chọn
                      </p>
                      tất cả{" "}
                      <span className="xl:block hidden">( {cartCount} )</span>
                    </div>
                  </button>

                  <button
                    className="w-16  mx-2 xl:block hidden"
                  ...
                  <Link
                    to="/products"
                    className="  text-center p-2 xl:block hidden"
                  >
          ...
          <div className="flex items-center mb-6">
            ...
            <div>
              <p className="text-lg font-semibold text-gray-800">
                {itemChooseSize?.product?.name}
              </p>
              <p className="text-sm text-gray-600">
                {itemChooseSize?.product?.description}
              </p>
              <p className="text-sm text-gray-600">
                Còn lại: {selectSize?.quantity || 0}
              </p>
            </div>
          </div>
          <div className="mb-6">
            ...
            <div className="flex mt-3 flex-wrap gap-3">
              {itemChooseSize?.product?.sizes?.map((item) => {
                ...
                const buttonClasses = `px-4 py-2 bg-white text-black rounded-md shadow-sm ${
                ...
                return (
                  <button
                    ...
                    className={
                      isAvailable ? buttonClasses : disabledButtonClasses
                    }
                  >
                    {item?.size}
                  </button>
