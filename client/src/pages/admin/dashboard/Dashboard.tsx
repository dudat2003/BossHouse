...
const Dashboard = () => {
  ...
  const [yearChance, setYearChance] = useState(
    ...
    datasets: [
      {
        ...
        data: Object.values(category).map((item) =>
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        const orders = resAll?.data?.filter(
          (i) => i?.products[0].status === "Đã Nhận Hàng"
        );
        // console.log(orders)
        ...
        orders.forEach((order) => {
          ...
          if (profitByMonth[monthKey] != undefined) {
            profitByMonth[monthKey] += order.products.reduce(
              (totalProfit, product) => {
                // Kiểm tra xem mỗi sản phẩm có thuộc tính initPriceProduct và initImportPriceProduct không
                if (
                  product.initPriceProduct &&
                  product.initImportPriceProduct
                ) {
                  // Tính lợi nhuận của sản phẩm
                  const profit =
                    product.initPriceProduct - product.initImportPriceProduct;
  ...
  useEffect(() => {
    ...
    dataOrder
      ?.filter((i) => i.products[0].status == "Đã Nhận Hàng")
      .forEach((order) => {
    ...
    const top5Users: [string, UserTotal][] = Object.entries<UserTotal>(
      userTotal
    )
                    ...
                    <div>
                            ...
                            <li key={index}>
                              <Link to={`/blogDetail/${item?._id}`}>
                                <div className="flex gap-x-[12px] py-2">
                                  <div>
                                    <img
                                      className="w-[80px] h-[60px] object-cover mt-1"
                                      src={item.imageTitle}
                                      alt="img-blog"
                                    />
                                  </div>
                      ...
                      <Link to={"/blog"}>
                        <div className="text-center my-3">
                          <button className="font-medium hover:text-[blue] hover:underline">
                            Xem thêm
                          </button>
                        </div>
                      </Link>
                ...
                <div className="bg-white p-6 rounded-lg shadow-md flex flex-col">
                  <Link to={"/admin/customer"}>
                    <h2 className="text-2xl font-bold mb-4">
                      Xếp hạng người dùng có lượng chi tiêu cao nhất
                    </h2>
                    <ul>
                      {topPrice?.map((user, index) => {
                        ...
                        return (
                          <li
                            key={index}
                            className="flex justify-between items-center py-3 border-b border-gray-200 transition-colors duration-300 hover:bg-gray-50"
                          >
                            ...
                            <div className="font-semibold text-gray-700">
                              {user[1]?.totalPrice.toLocaleString()} đ
                            </div>
                          </li>
                        );
                      })}
